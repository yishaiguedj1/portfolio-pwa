...
ok(/let R = R0, rho = ringAt\(R0, 0\.72\), ok = false;/.test(src) && /g\.y = Math\.sin\(t\) \* rho \+ dyShift;/.test(src) && /const logoOn = \(\) =>/.test(src) && /for \(let t = g\.a - 0\.35; t <= g\.a2 \+ 0\.35; t \+= 0\.02\)/.test(src) && /if \(g === segs\[0\]\) \{ th = g\.mid;/.test(src) && /for \(const sc of \[1, 0\.9, 0\.82, 0\.76, 0\.7\]\)/.test(src) && /g\.skip = false;/.test(src) && !/g\.skip = shown\.some/.test(src), 'v185/v188: שרשרת בסדר השעון — הגדולה מול הפרוסה; כל השאר: הלוגו על הפרוסה, הכי נגד כיוון השעון שאפשר בלי נגיעה (הבועה מותר לה לגלוש); תוויות קטנות יותר לפני יציאה מהטבעת; אף תווית לא נשמטת');
console.log('\n' + n + ' בדיקות עברו');
